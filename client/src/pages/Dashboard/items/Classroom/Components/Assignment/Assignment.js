...
export default ({ type, data, id, setPopup, getAssignments, amountAssigned = 0 }) => {
    ...
    return (
        <div className="classroom-homework__outer">
            <div className={"classroom-homework__item align-center " + ( open && "classroom-homework--toggled" )} onClick={() => setOpen(!open)}>
                ...
                <i class="classroom-homework__edit fas fa-ellipsis-v" onClick={(e) => (e.stopPropagation(), setPopup(<ContentDelete setPopup={setPopup} deleteContent={() => deleteHomework(id, uid)}/>))}></i>
            </div>
